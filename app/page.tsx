...
function OperationsWorkspace({ view, setNotice, leads, originalData }: OperationsWorkspaceProps) {
  ...
  if (view === "Pedidos") {
    const searchedContracts = records.filter((event) => `${event.title} ${event.client} ${event.date} ${event.project}`.toLowerCase().includes(contractQuery.toLowerCase()));
    return <section className="orders-layout"><div className="section-heading"><div><h2>Contratos e fichas</h2><p>Localize por cliente, ficha, data ou serviço.</p></div><label className="search-field"><Search size={16} /><input value={contractQuery} onChange={(event) => setContractQuery(event.target.value)} placeholder="Buscar cliente, data ou ficha" /></label></div><div className="document-flow"><article><FileText size={23} /><h3>{searchedContracts.length} fichas</h3><p>Eventos encontrados na busca.</p></article><ChevronRight /><article><ClipboardList size={23} /><h3>Contratos</h3><p>PDFs, assinatura e valores na mesma lista.</p></article><ChevronRight /><article><ReceiptText size={23} /><h3>Histórico</h3><p>Cliente, data e pagamento em um só lugar.</p></article></div><div className="contract-directory">{searchedContracts.length ? searchedContracts.map((event) => <article key={event.id} className="contract-row"><div><strong>{event.client}</strong><span>{event.title} · {event.date} · {event.time}</span><small>{event.project}</small></div><div><span className="stage-pill">{event.contractSigned ? "Assinado" : event.contract}</span><b>{event.value}</b></div><button className="outline-button" onClick={() => { setOpenEventId(event.id); setNotice(`Ficha localizada: ${event.title}. Abra Agenda para editar todos os dados.`); }}>Localizar ficha <ChevronRight size={15} /></button><button className="outline-button" disabled={originalData.saving} onClick={() => void moveToTrash(event)}><Trash2 size={15} /> Lixeira</button>{event.contractUrl && <a className="outline-button" href={event.contractUrl} target="_blank" rel="noreferrer">PDF <ExternalLink size={15} /></a>}</article>) : <div className="availability-note"><Search size={18} /><span>Nenhuma ficha encontrada para esta pesquisa.</span></div>}</div></section>;
  }
  const receivedProposals = flowRequests.filter((request) => request.status !== "Lixeira" && String(request.tipoRecebido || "proposal") !== "contract");
  return <section className="orders-layout"><div className="section-heading"><div><h2>Propostas</h2><p>Uma trilha clara, sem duplicar informações.</p></div><button className="primary-button" onClick={() => setNotice("Novo documento preparado para o fluxo do Gerador.")}><Plus size={17} /> Criar proposta</button></div><div className="document-flow"><article><FileText size={23} /><h3>1. Proposta</h3><p>Cliente escolhe itens no Gerador.</p></article><ChevronRight /><article><ClipboardList size={23} /><h3>2. Pedido</h3><p>Itens e valor ficam organizados.</p></article><ChevronRight /><article><ReceiptText size={23} /><h3>3. Contrato e recibo</h3><p>Assinatura e pagamento viram histórico.</p></article></div><div className="contract-directory">{receivedProposals.length ? receivedProposals.map((request) => { const contractor = (request.dadosContratante || {}) as Record<string, unknown>; const commercial = (request.dadosComerciais || {}) as Record<string, unknown>; return <article key={String(request.id)} className="contract-row"><div><strong>{String(contractor.nome || "Cliente")}</strong><span>{String(request.tipoEvento || "Evento")} · {String((request.dadosEvento as Record<string, unknown> | undefined)?.data || "Data a confirmar")}</span><small>{String(commercial.servico || "Serviço a confirmar")}</small></div><div><span className="stage-pill">Recebida do Flow</span><b>{Number(commercial.valorTotal || 0) ? `R$ ${Number(commercial.valorTotal).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}` : "Valor a confirmar"}</b></div><button className="outline-button" disabled={originalData.saving} onClick={() => void moveFlowRequestToTrash(request as Record<string, unknown>)}><Trash2 size={15} /> Lixeira</button></article>; }) : <div className="availability-note"><FileText size={18} /><span>As propostas enviadas pelo Flow aparecerão aqui.</span></div>}</div></section>;
}
