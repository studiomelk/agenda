...
import {
  ...
  , ShoppingBag, Images, FormInput, Link2, Send, BookOpen, ImageIcon, PanelsTopLeft
} from "lucide-react";
...
const navigation: [LucideIcon, string][] = [
  ...
  [Images, "Entregas"], [BookOpen, "Álbuns"], [FormInput, "Formulários"], [CircleDollarSign, "Financeiro"], [PanelsTopLeft, "Gerador"], [Trash2, "Lixeira"], [ShieldCheck, "Importação"],
];

const generatorUrl = (process.env.NEXT_PUBLIC_GERADOR_URL || "https://gerador-studio-melk.vercel.app").replace(/\/$/, "");
  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        </> : activeView === "Importação" ? <ImportWorkspace setNotice={setNotice} onUnlock={setAccessKey} originalData={originalData} /> : activeView === "Gerador" ? <GeneratorWorkspace /> : <OperationsWorkspace view={activeView} setNotice={setNotice} leads={leads} originalData={originalData} />}
      </section>
    </main>
  );
}
...
function GeneratorWorkspace() {
  ...
  const embeddedUrl = `${generatorUrl}/?embed=1&host=flow`;
  return <section className="generator-workspace">
    <header className="generator-heading">
      <div><span className="generator-mode"><PanelsTopLeft size={14} /> Modo integrado</span><h2>Trabalhe sem sair do Flow</h2><p>É o mesmo Gerador da versão independente; seus recursos e atualizações permanecem iguais.</p></div>
      <div className="generator-actions"><button className="outline-button" type="button" onClick={() => setFrameKey((value) => value + 1)}><RotateCcw size={16} /> Recarregar</button><a className="primary-button" href={generatorUrl} target="_blank" rel="noreferrer">Abrir separado <ExternalLink size={16} /></a></div>
    </header>
    <div className="generator-frame-shell">
      <iframe key={frameKey} className="generator-frame" src={embeddedUrl} title="Gerador Studio Melk" allow="clipboard-read; clipboard-write" referrerPolicy="strict-origin" />
    </div>
  </section>;
}
...
function ImportWorkspace({ setNotice, onUnlock, originalData }: { setNotice: (message: string) => void; onUnlock: (password: string) => void; originalData: OriginalStudioData }) {
  ...
  async function saveConnection() {
    setConnecting(true);
    try {
      const currentCode = localStorage.getItem("studio-melk-flow-pair-code") || pairCode;
...
function FormsWorkspace({setNotice}:{setNotice:(message:string)=>void}) {
  ...
  const publicUrl=`https://studio-melk.vercel.app/formulario/${selected.id}/${formKind}`;
